...
import { createMentorSchema, updateMentorSchema, type CreateMentorInput, type UpdateMentorInput } from "@/lib/validations/mentor";
  ...
  return (
    <div className="max-w-md bg-white border border-zinc-200 rounded-xl shadow-card p-6 space-y-5">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h3 className="font-semibold text-zinc-900 text-[15px]">Mentor created successfully</h3>
          <p className="text-[13px] text-zinc-500 mt-0.5">Share these login credentials with the mentor.</p>
        </div>
      </div>
        ...
        {[
          ...
          <div key={key} className="flex items-center gap-3 bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-3">
            ...
            <button
              ...
              className="h-7 w-7 rounded-lg flex items-center justify-center text-zinc-400 hover:text-zinc-700 hover:bg-zinc-200 transition-colors duration-150 shrink-0"
            >
      ...
      <p className="text-[12px] text-amber-600 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2">
        Credentials have been emailed to the mentor. They must change their password on first login.
      </p>

      <div className="flex gap-2 pt-1">
        <Button size="sm" onClick={onBack} className="rounded-lg bg-indigo-600 hover:bg-indigo-700 text-[13px]">
          Back to Mentors
        </Button>
        <Button size="sm" variant="outline" onClick={onAddAnother} className="rounded-lg text-[13px]">
          Add Another
        </Button>
      </div>
    </div>
...
export function MentorForm({ mode, mentorId, defaultValues }: MentorFormProps) {
  ...
  const schema = mode === "create" ? createMentorSchema : updateMentorSchema;
  ...
  async function onSubmit(data: CreateMentorInput | UpdateMentorInput) {
    setError("");
    ...
    const url = mode === "create" ? "/api/admin/mentors" : `/api/admin/mentors/${mentorId}`;
    ...
    if (mode === "create") {
      setCreatedCredentials({ userId: json.userId, password: (data as CreateMentorInput).initialPassword });
    } else {
  ...
  if (createdCredentials) {
    return (
      <CredentialBox
        ...
        onBack={() => { setCreatedCredentials(null); router.push("/admin/mentors"); router.refresh(); }}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2 space-y-1.5">
            ...
            <Input {...register("name")} placeholder="Dr. Jane Smith" className={inputCls} />
            {errors.name && <p className="text-xs text-red-500">{errors.name.message as string}</p>}
          </div>

          <div className="space-y-1.5">
            <Label className="text-[13px] font-medium text-zinc-700">Email *</Label>
            <Input type="email" {...register("email")} placeholder="jane@example.com" className={inputCls} />
          ...
          <div className="space-y-1.5">
            <Label className="text-[13px] font-medium text-zinc-700">Specialization</Label>
            <Input {...register("specialization")} placeholder="Web Development" className={inputCls} />
          </div>

          <div className="space-y-1.5">
            <Label className="text-[13px] font-medium text-zinc-700">Department</Label>
            <Input {...register("department")} placeholder="Engineering" className={inputCls} />
          </div>

          <div className="col-span-2 space-y-1.5">
            <Label className="text-[13px] font-medium text-zinc-700">Bio</Label>
            <Textarea
              {...register("bio")}
              placeholder="Brief bio about this mentor…"
              rows={3}
        ...
        <div className="flex gap-2.5 pt-1 border-t border-zinc-100">
          ...
          <Button type="button" variant="outline" onClick={() => router.push("/admin/mentors")} className="rounded-lg text-[13px] h-9">
            Cancel
          </Button>
        </div>
      </form>
