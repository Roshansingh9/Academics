...
export function AppSidebar() {
  ...
  // While session is loading, infer role from pathname so the correct nav
  // renders immediately. Middleware already enforces that only the right role
  // can reach each path prefix, so this inference is always safe.
  const inferredRole = pathname.startsWith("/admin")
    ? "ADMIN"
    : pathname.startsWith("/mentor")
    ? "MENTOR"
    : "STUDENT";
  const effectiveRole = role ?? inferredRole;
  const nav = effectiveRole === "ADMIN" ? adminNav : effectiveRole === "MENTOR" ? mentorNav : studentNav;
        ...
        <div className="flex items-center gap-2.5 px-2 py-2 rounded-lg hover:bg-zinc-800/60 transition-colors duration-150 cursor-default">
          <div className="h-7 w-7 rounded-full bg-indigo-600/20 border border-indigo-500/30 flex items-center justify-center shrink-0">
            <span className="text-[10px] font-bold text-indigo-300">
              {session?.user?.userId?.slice(0, 2).toUpperCase() ?? inferredRole.slice(0, 2)}
            </span>
          </div>
          <div className="min-w-0 flex-1">
            <p className="text-[12px] font-medium text-zinc-300 truncate leading-none">
              {session?.user?.userId ?? "Loading..."}
            </p>
